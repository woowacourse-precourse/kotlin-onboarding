...
fun dupleCheck(str: String): ArrayList<Int> { // 처음으로 나오는 중복 문자들의 왼쪽을 index를 return
    ...
    for (idx in 1 until str.length) {
        ...
        if (str[idx - 1] != str[idx] && duple.isNotEmpty()) {
            return duple
        }
    }
    return duple
...
fun updateStr(str: String, duple: ArrayList<Int>): String { // 입력 duple을 토대로, 중복된 부분 제거
    if (duple.isNotEmpty()) {
...
fun solution2(cryptogram: String): String {
    var duple = arrayListOf(0)
    var result: String = cryptogram
    while (duple.isNotEmpty()) { // 중복된 문자가 나오지 않으면 종료
