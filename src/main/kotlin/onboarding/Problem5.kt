...
fun loopExchange(money : Int, resultList : MutableList<Int> ) : List<Int> {
    ...
    for(i in priceList) {
        ...
        mMoney = mList[1] // mList의 1번째에는 나머지 값, 즉 남은 금액이 들어가므로, 이를 활용해 다시 한번 루프를 돌리면 남은 금액에 그 다음번 필터가 몇 번 돌아갈지가 구해진다.
    }
    return resultList
}
...
private fun checkHowMuch(price : Int, priceList : Int) : List<Int> {
    return listOf(price / priceList, price % priceList) // 몫과 나머지 모두를 반환해줘야 하므로 List<Int> 형 반환을 선택했다.
}
