...
fun solution7(
    ...
    friends.forEach { friend ->
        ...
        if (idMap.containsKey(idA)) {
            idMap[idA]?.add(idB)
        } else {
            idMap[idA] = mutableListOf(idB)
        }

        if (idMap.containsKey(idB)) {
            idMap[idB]?.add(idA)
        } else {
            idMap[idB] = mutableListOf(idA)
        }
    }
    ...
    visitors.filter { idMap[user]?.contains(it) == false }.forEach { visitor ->
        if (scoreMap[visitor] != null) {
            scoreMap[visitor] = scoreMap.getValue(visitor) + 1
        } else {
            scoreMap[visitor] = 1
        }
    }

    recommendFriends = scoreMap.toList()
