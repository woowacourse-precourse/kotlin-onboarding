...
fun findShare(friends: List<List<String>>, user_friends: List<String>, user: String): MutableMap<String, Int>{
    ...
    for (people in user_friends) {
        for ((first, last) in friends) {
            ...
            if (first == people) {
                if (!cnt_friend.containsKey(last)) cnt_friend.put(last, 10)
                else cnt_friend[last] = cnt_friend[last]!! + 10
            }
            if (last == people) {
                if (!cnt_friend.containsKey(first)) cnt_friend.put(first, 10)
...
fun visitPeople(share_friends: MutableMap<String, Int>, visit: String): MutableMap<String, Int>{
    if (!share_friends.containsKey(visit)) share_friends.put(visit, 1)
...
fun solution7(
    ...
    val user_friends = findFriends(friends, user)
    var share_friends = findShare(friends, user_friends, user)
    for (visit in visitors) {
        if (user_friends.contains(visit)) continue
        share_friends = visitPeople(share_friends, visit)
    }
    share_friends = share_friends.toList().sortedByDescending { it.second }.toMap() as MutableMap
    val result:List<String> = share_friends.keys.toList()
