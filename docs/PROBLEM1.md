...
* 입력받은 숫자 리스트에 대해 일급 컬렉션을 구현한다.
  - 입력된 값에 대한 예외처리를 구현한다. (예외사항의 경우 throw 처리)
    + 왼쪽 값이 홀수인지 검증한다.
    + 오른쪽 값이 짝수인지 검증한다.
    + 오른쪽 값에서 왼쪽 값을 뺀 값이 1인지 검증한다.
  - 각 자리 수를 모두 더한 값을 반환하는 메서드를 구현한다.
  - 각 자리 수를 모두 곱한 값을 반환하는 메서드를 구현한다.
